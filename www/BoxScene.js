...
btest.setUpdate(
    function (scene, gTouchOn, gTouchOnOff, gTouchX, gTouchY, gTouchMoveX, gTouchMoveY, gTouchOffX, gTouchOffY) {
        ...
        if (gTouchOnOff) {
            ...
            //scene.addDemo(sGame.createRectDemo('rd' + num, { 'res': 'null', 'size': { 'w': '50', 'h': '50' }, 'pos': { 'x': gTouchOffX, 'y': gTouchOffY }, 'scale': { 'x': '1', 'y': '1' }, 'alpha': '1', 'angle': 0, 'wp': { 'onWorld': 'true', 'fixedRotation': 'false', 'density': '5', 'friction': '0.3', 'restitution': '0.5', 'bodytype': '2' }, 'shape': { 'color': '#00EE00', 'fill': 'false', 'line': '5' } }));
            num++;
        }
    }
);
...
rd.setUpdate(
    function (scene, gTouchOn, gTouchOnOff, gTouchX, gTouchY, gTouchMoveX, gTouchMoveY, gTouchOffX, gTouchOffY) {
        ...
        if (gTouchOnOff) {
            // 自分自身がタッチされた？
            if (this.existWorldPoint(scene, gTouchOffX, gTouchOffY)) {
                this.bodyApplyForce(10, 0, this.pos.x, this.pos.y);
            }
        }
    }
);
    ...
    function (scene, gTouchOn, gTouchOnOff, gTouchX, gTouchY, gTouchMoveX, gTouchMoveY, gTouchOffX, gTouchOffY) {
        ...
        if (gTouchOnOff) {
            if (this.isHit(gTouchOffX, gTouchOffY, 1, 1)) {
                ...
                for (let k in scene.contactdict) {
                    //console.log(k);
                }
            }
            ...
            // 自分自身がタッチされた？
            if (this.existWorldPoint(scene, gTouchOffX, gTouchOffY)) {
                if(this.contactDemoInWorld(scene, scene.demo["rd"])){
                    console.log("■がふれてる！");
                }
            }
        }
